// MicroGlut is a stripped-down reimplementation/rejuvenation of the classic GLUT/FreeGLUT library. By Ingemar Ragnemalm 2012-2022.

// Note: MicroGlut is not GLUT! It is smaller, simpler, skips all outdated parts, adds
// a few new features and simplifies the API in some parts. It has no intention of being
// 100% compatible, but to be a modernized version suitable for course work.

// This is the Linux version. There is also a Mac version and a Windows version.

// Why do we use MicroGlut?
// 1) Transparency! Including it as a single source file makes it easy to see what it does. Also, no hassle with installations.
...
// Please note that this is still under development. A lot of functionality is still missing.
// If you add/change something of interest, especially if it follows the GLUT API, please sumbit
// to me so I can consider adding that to the official version. OTOH, if you see possibilities
// for further simplification, that canbe even more interesting!
...
// 170406: Added "const" to string arguments to make C++ happier. glutSpecialFunc and
// glutSpecialUpFunc are now officially supported - despite being deprecated. (I recommend
// that you use the same keyboard func for everything.) Added support for multiple mouse
...
// 220112: Added GLUT_KEY_SHIFT to conform with other platforms.
// 230508: Added timer removal.
...
char glutKeyIsDown(unsigned char c)
{
	if (c == GLUT_KEY_SHIFT)
	{
		return gKeymap[GLUT_KEY_LEFT_SHIFT] || gKeymap[GLUT_KEY_RIGHT_SHIFT];
	}
//	if (c == GLUT_KEY_CTRL)
//	{
