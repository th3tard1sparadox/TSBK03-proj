...
// By Ingemar Ragnemalm 2012-2021
...
// 220111: Added support for modifier keys in glutKeyIsDown. (Old GLUT uses glutGetModifiers but MicroGlut is simpler!)
// 220201: Minor change with updatePending, needs more testing.
// 220215: Bringing in the window in focus repeatedly to avoid losing it.
// Doesn't look like the best solution but losing focus is not OK.
// 220223: Unsigned char in doKeyboardEvent.
// 220511: glutPostRedisplay does not order a redraw if update is pending = called from inside update.
// Resize now sets updatePending to 2 - seems to fix the problem!
// 220817: No, but removed clearDrawable from drawRect did. Is it fine now?
// 230508: glutRepeatingTimer will now stop the timer when passed 0. Finally removed glutRepeatingTimerFunc.
...
-(void)drawRect:(NSRect)rect
{
	// I have had a lot of problems with the drawing, either it disappears when
	// I change device or glReadPixels fails (gets nothing).
	// Possible solution: Only clearDrawable the first time after a resize!
	// Also after change of device!
	static int idiot = 0;
	static NSScreen *lastscreen = NULL;

	if ([window screen] != lastscreen || idiot)
	{
	...
	if (([theView frame].size.width != lastWidth) || ([theView frame].size.height != lastHeight))
	{
		...
		//[m_context clearDrawable]; // Weird; I need TWO of these.
		// Even weirder: Removed it again. Does it work now?
...
//		}
	...
	if (gDisplay != NULL)
		gDisplay();
	if (updatePending > 0)
		updatePending -= 1; // Finally helped!
...
int gWindowInitPosX = 10;
int gWindowInitPosY = 50;
...
{
	NSRect frame = NSMakeRect(gWindowInitPosX, NSScreen.mainScreen.frame.size.height - gWindowInitPosY-gWindowInitHeight, gWindowInitWidth, gWindowInitHeight);
...
// Moved back to main loop since it tends to lose focus
	[window makeFirstResponder: view]; // Added 130214
...
void glutPositionWindow(int x, int y)
{
	...
	r = [window frame];
	r.origin.x = x;
	r.origin.y = NSScreen.mainScreen.frame.size.height - gWindowInitPosY - r.size.height;
	[window setFrame: r display: true];
}
